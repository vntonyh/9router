    ...
    <div className="flex flex-col gap-2">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card className="px-4 py-3 flex flex-col gap-1">
          ...
          <span className="text-2xl font-bold">{fmt(stats.periodRequests)}</span>
          {stats.totalRequests > stats.periodRequests && (
            <span className="text-[10px] text-text-muted">Lifetime: {fmt(stats.totalRequests)}</span>
          )}
        </Card>
        <Card className="px-4 py-3 flex flex-col gap-1">
      ...
      {isTruncated && (
        <p className="text-[11px] text-text-muted">
          Token/cost stats based on last {fmt(stats.maxHistory)} retained requests. Set <code className="text-[10px]">MAX_USAGE_HISTORY</code> env to increase.
        </p>
      )}
    </div>
  );
